...
export class AuthController {
   ...
   async signUp(@Body() createUserDto:CreateUserDto, @Res({passthrough:true}) res){
    ...
    if(isUser){
     ...
     const accessTokenExpiration = parseInt(process.env.JWT_ACCESS_TOKEN_EXPIRATION_TIME!, 10);
    const refreshTokenExpiration = parseInt(process.env.JWT_REFRESH_TOKEN_EXPIRATION_TIME!, 10);
         // @ts-ignore
   ...
   @Post('/login')
   async login(@Body() loginUserDto:LoginUserDto, @Res({passthrough:true}) res:any){
   ...
   async googleAuth(@Req() req, @Res({passthrough:true}) res){
    ...
    const {access_token,refresh_token} = await this.authService.generateAuthTokens({username:result.email, sub:result.id})
...
/*     // redirect
   ...
   async refreshToken(@Req() req:Request, @Res({passthrough:true}) res){
    ...
    const {accessToken,refreshToken} = await this.authService.rotateRefreshToken(userId,oldRefreshToken)

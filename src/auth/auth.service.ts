...
export class AuthService {
    ...
    async signUp(createUserDto:CreateUserDto){
        ...
        const refreshTokenExpirationSeconds = parseInt(process.env.JWT_REFRESH_TOKEN_EXPIRATION_TIME || '604800000',10)
        await this.redisService.set(refreshTokenKey,refresh_token,refreshTokenExpirationSeconds)
    ...
    async rotateRefreshToken(userId:number, oldRefreshToken:string){
        ...
        try {
            ...
            await this.redisService.del(refreshTokenKey)
            throw new UnauthorizedException("Invalid refresh token signature or expiration. Please login again")
        }
        ...
        await this.redisService.set(refreshTokenKey,newRefreshToken,refreshTokenExpirationSeconds)
